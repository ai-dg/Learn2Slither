...
class Game:
    ...
    def ft_get_vision_snake(self):
        # Initialize container every time
        self.vision = {"up": [], "down": [], "left": [], "right": []}

        # If snake vanished (e.g., red apple to 0), mark game over and return empty vision
    ...
    def ft_move_snake(self, direction: str, terminal: str):
        ...
        try:
            ...
            # Dernière ligne de défense : on termine proprement la session
            self.is_over = True
            # (Optionnel) si tu veux vraiment signifier "mort sur erreur":
            # code = self.WALL
            self.ft_show_terminal_movements(code, direction, terminal)
            return code, True

        # ⛔️ NE CALCULE PAS LA VISION SI LA PARTIE EST FINIE OU SI LE SERPENT EST VIDE
        if self.is_over or len(self.snake) == 0:
            self.ft_show_terminal_movements(code, direction, terminal)
            return code, True

        # OK: on est vivant => maintenant seulement on met à jour la vision
        self.ft_get_vision_snake()
        self.ft_show_terminal_movements(code, direction, terminal)
    ...
    def ft_move_snake_detail(self, direction: str):
        ...
        if not self.is_over and len(self.snake) > 0:
            self._enforce_apple_invariants()
            self._ensure_single_head()
        return tile
