...
class DeepQNetwork:
    ...
    ACTIONS = ['UP', 'DOWN', 'LEFT', 'RIGHT']  # Actions en majuscules pour AI/board.py
    ...
    def ft_encode_vision(self, vision):
        """Encoder la vision simple de AI/board.py (4 caractères) en vecteur"""
        # vision est une string de 4 caractères: "WHSG" par exemple
        parts_state = []
        ...
        for char in vision:
            parts_state.append(char_to_num.get(char, 0.0))
    ...
    def ft_choose_action(self, state_vec):
        if len(self.experience) < self.warmup_steps:
            # Pendant warmup, éviter les pommes rouges visibles
            return self.ft_choose_safe_action(state_vec)
        
        q_values = self.q_network.predict(state_vec[None, :], verbose=0)[0]

        if random.random() < self.epsilon:
            # Exploration : éviter les pommes rouges visibles
            return self.ft_choose_exploration_action(state_vec, q_values)
        return int(np.argmax(q_values))
    
    def ft_choose_safe_action(self, state_vec):
        """Choisir une action qui évite les pommes rouges visibles"""
        safe_actions = []
        red_apple_actions = []
        
        for i, action in enumerate(self.ACTIONS):
            if i < len(state_vec):
                if state_vec[i] == 5.0:  # Pomme rouge visible
                    red_apple_actions.append(i)
                else:
                    safe_actions.append(i)
        
        # Éviter les pommes rouges si possible
        if safe_actions:
            return random.choice(safe_actions)
        else:
            # Si toutes les directions ont des pommes rouges, choisir aléatoirement
            return random.randrange(self.num_actions)
    
    def ft_choose_exploration_action(self, state_vec, q_values):
        """Exploration qui privilégie les pommes vertes et évite les pommes rouges"""
        green_apple_actions = []
        safe_actions = []
        red_apple_actions = []
        ...
        for i, action in enumerate(self.ACTIONS):
            if i < len(state_vec):
                if state_vec[i] == 4.0:  # Pomme verte visible
                    green_apple_actions.append(i)
                elif state_vec[i] == 5.0:  # Pomme rouge visible
                    red_apple_actions.append(i)
                else:
                    safe_actions.append(i)
        
        # Priorité 1: Pommes vertes (90% de chance)
        if green_apple_actions and random.random() < 0.9:
            return random.choice(green_apple_actions)
        
        # Priorité 2: Actions sûres
        if safe_actions:
            return random.choice(safe_actions)
        
        # Dernière option: choisir aléatoirement (même les pommes rouges)
        return random.randrange(self.num_actions)
    ...
    def ft_calculate_reward(self, reward_from_board, is_over, next_state):
        """Calculer la récompense pour AI/board.py"""
        # AI/board.py fournit déjà la récompense de base
        reward = reward_from_board
        
        if is_over:
            reward = -100.0  # Pénalité forte pour la mort
        
        # Récompense pour voir une pomme verte dans la vision
        green_apples_seen = 0
        red_apples_seen = 0
        
        for i in range(len(next_state)):
            if next_state[i] == 4.0:  # Pomme verte visible
                green_apples_seen += 1
            elif next_state[i] == 5.0:  # Pomme rouge visible
                red_apples_seen += 1
        ...
        reward -= red_apples_seen * 10.0   # Pénalité TRÈS FORTE pour chaque pomme rouge visible
    ...
    def ft_compute_loss(self):
        ...
        if self.debug_train and (self.train_step_count % self.log_every == 0):
            print(f"[train:{self.train_step_count}] "
                  f"len_exp={len(self.experience)} | "
                  f"y_targets mean={float(tf.reduce_mean(y_targets)):.2f} | "
                  f"q_values mean={float(tf.reduce_mean(q_values)):.2f}")
    ...
    def ft_interaction(self, sessions: int):
        ...
        # --- init DQN ---
        self.experience = deque(maxlen=self.memory_size)
        self.states = self.ft_encode_vision(self.board.get_snake_vision())
        ...
        for session_id in range(1, sessions + 1):
            ...
            start_len = len(self.board.snake)
            start_head = tuple(self.board.snake[0])
            ep_return = 0.0
            last_loss = None
            ...
            try:
                ...
                for step in range(max_num_timesteps):
                    ...
                    game_continues, reward_from_board = self.board.move_snake(action_str)
                    is_over = not game_continues

                    next_state = self.ft_encode_vision(self.board.get_snake_vision())
                    
                    reward = self.ft_calculate_reward(reward_from_board, is_over, next_state)
                    # print(f"Reward from board: {reward_from_board}")
                    # print(f"Final reward: {reward}")
                    if reward_from_board > 0: green_hits += 1
                    if reward_from_board < 0 and not is_over: red_hits += 1
                    ep_return += reward
                    ...
                    len_after = len(self.board.snake)

                    if len_after > max_length_global:
                        max_length_global = len_after

                    last_event_tile = "GREEN" if reward_from_board > 0 else "RED" if reward_from_board < 0 else "EMPTY"
            ...
            self.len_hist.append(len(self.board.snake))
